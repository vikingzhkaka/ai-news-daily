...
## 推送失败提醒
若某次推送至 GitHub 失败（如 gh 未登录、网络不可达），自动化会保留本地最新 index.html 并在下次运行重试；若已在本机配置 SMTP 凭据，还会额外向 vikingzh@hotmail.com 发送一封失败提醒邮件（凭据仅存于本机，不入库）。

> 注意：Outlook/Hotmail 已禁用 SMTP 基础认证，不能作为发信端。请用仍支持 SMTP 授权的邮箱（如 QQ/163）作为发信账户，收件人仍填 hotmail 地址即可正常收到。凭据格式见本机 `.workbuddy/mail_creds.json`。
